...
export class AdminUserFormUnroutedComponent implements OnInit {
  ...
  initializeForm(oUser: IUser) {
    this.userForm = this.oFormBuilder.group({
      ...
      identificador_empresarial: [oUser.identificador_empresarial],
      nombre_empresa: [oUser.nombre_empresa, [Validators.required, Validators.minLength(6), Validators.maxLength(15), Validators.pattern('^[a-zA-Z0-9]+$')]],
